...
6. Cross-cutting heavy reasoning (architecture design, complex debugging, algorithm design) with no single domain owner? Use the deep-reasoner subagent
7. Mechanical, low-ambiguity execution (boilerplate, formatting, repetitive scaffolding, mechanical test writing)? Use the fast-worker subagent
...
### Reasoning-Tier Agents (2) — model-fixed complexity tier, cross-cutting (not domain-scoped)

deep-reasoner (model: opus, effort: high — architecture design, complex debugging, algorithm design/analysis, system design tradeoffs)
fast-worker (model: sonnet, effort: high — boilerplate, mechanical test writing, formatting, repetitive scaffolding)
...
### Agentic Development Pipeline Guideline

@development_pipeline_guideline.md

### Global Tacit Knowledge (Core Skills)

Referenced by `development_pipeline_guideline.md` Section 1 as the global tacit-knowledge ruleset bound to this pipeline:

@core-skills/CLAUDE_TACIT_KNOWLEDGE.md
@core-skills/prevent-supply-chain-attack.md
@core-skills/CONTRIBUTING.md

### Recommended Workflow Chains

Project-phase-driven command chains. MoAI selects the appropriate chain (new feature / bug fix / refactoring / docs update) based on the current work context to keep quality management clean.

@core-skills/RECOMMEND_WORKFLOW_CHAIN.md
...
- **UltraThink** (`ultrathink` keyword): Sets `effort: max` in Claude Code v2.1.110+. On Opus 4.7 and later (including Opus 5), this triggers Adaptive Thinking (dynamically allocated reasoning tokens, no fixed budget_tokens). For older models, maps to extended thinking with high budget. No MCP dependency — compatible with all APIs. Do NOT confuse with `--deepthink`.
- **Adaptive Thinking** (Opus 4.7 and later, including Opus 5): the current Opus thinking mode. Unlike earlier models that use `budget_tokens`, Adaptive Thinking dynamically allocates reasoning based on task complexity. Triggered via `effort` level (high/xhigh/max) — not by `budget_tokens`. See Skill("moai-workflow-thinking").
...
## 18. SPEC Review Pipeline

Three-stage document review for SPEC sets (`spec.md` + `acceptance.md` + `plan.md` + `spec-compact.md`). Applied from SPEC-UI-001 onward; earlier SPECs were reviewed manually.
