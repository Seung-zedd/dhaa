...
**Pair-layout calibration**: When the SPEC directory uses the pair layout (EARS-format requirements in spec.md, Given/When/Then scenarios in acceptance.md — the house layout per SPEC-DB-001 v0.2.0 template), MP-2 applies to the normative requirement clauses in spec.md only. acceptance.md Given/When/Then scenarios are the house acceptance format and MUST NOT be judged as "Given/When/Then test scenarios mislabeled as EARS" — that phrasing in the M3 rubric (Score 0.50 band) refers to GWT text appearing inside spec.md requirement sections, not to acceptance.md's own scenario format. Only flag GWT-as-EARS when GWT text appears inside spec.md requirement sections.
...
<!-- LOCAL-EXT spec-review-pipeline BEGIN -->
**House frontmatter schema calibration (founder-confirmed 2026-07-10, SPEC-UI-001 review B-5)**: This project's canonical SPEC frontmatter schema is: id (string), version (string), status (string), **created** (ISO date string), **updated** (ISO date string), **author** (string), priority (string), **issue_number** (integer). `created`/`updated` replace the generic rubric's `created_at`; `labels` is intentionally NOT adopted. Judge MP-3 against this house schema — do NOT flag `created` (vs `created_at`) or a missing `labels` field as a defect. A genuinely missing house field (e.g., no `version` or no `updated`) is still FAIL.
<!-- LOCAL-EXT spec-review-pipeline END -->
...
<!-- LOCAL-EXT spec-review-pipeline BEGIN -->
### Group 7: Cross-Document Consistency (pair layout)

- CD-1: REQ ↔ Scenario bidirectional coverage — every REQ in spec.md is exercised by at least one acceptance.md Scenario or Edge Case; every Scenario/Edge Case traces to at least one existing REQ. Both directions; report every gap.
- CD-2: Content correspondence — for each mapped REQ↔Scenario pair, the scenario actually tests the REQ's normative content (a label citation with mismatched substance is a defect, not coverage).
...
<!-- LOCAL-EXT spec-review-pipeline BEGIN -->
## REQ ↔ Scenario Coverage Map
| REQ | Covering Scenarios / Edge Cases | Gap? |
|-----|--------------------------------|------|
(one row per REQ; "Gap?" = NONE or description)

Orphaned scenarios (no valid REQ basis): {list or "none"}
Cross-document drift found (CD-3/CD-4/CD-5): {list with file:line pairs or "none"}
<!-- LOCAL-EXT spec-review-pipeline END -->
